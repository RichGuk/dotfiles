...
vim.keymap.set('n', '<leader>es', ':sp <C-R>=expand("%:p:h") . "/" <CR>', { desc = 'Open file in same dir in split' })
vim.keymap.set('n', '<leader>ef', ':e <C-R>=expand("%:p:h") . "/" <CR>', { desc = 'Open file in same dir' })
vim.keymap.set('n', '<leader>ev', ':vsp <C-R>=expand("%:p:h") . "/" <CR>', { desc = 'Open file in same dir in vsplit' })
vim.keymap.set('n', '<leader>et', ':tabe <C-R>=expand("%:p:h") . "/" <CR>', { desc = 'Open file in same dir in tab' })
...
vim.api.nvim_create_autocmd('LspAttach', {
  ...
  callback = function(event)
    ...
    vim.keymap.set('n', '<leader>ca', vim.lsp.buf.code_action, { noremap = false, buffer = event.buf, desc = 'Code action' })
    vim.keymap.set('n', '<leader>vd', vim.diagnostic.open_float, { buffer = event.buf, desc = 'Diagnostic float' })
  end
})
