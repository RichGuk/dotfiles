...
update_track() {
    ...
    if [ $PLAYER_STATE = "Playing" ]; then
        ...
        icon.padding_right=10 label.color=$TEXT icon.color=$TEXT label.drawing=yes background.color=$SPOT_BACKGROUND

    elif [ $PLAYER_STATE = "Paused" ]; then
        sketchybar --set $NAME label.color=$YELLOW
    elif [ $PLAYER_STATE = "Stopped" ]; then
        sketchybar --set $NAME label.color=$MAROON
    else
        sketchybar --set $NAME background.color=$SPOT_BACKGROUND
    fi
}
